...
DEFAULT_INITIAL_IPS = [
    ("38.20.140.122", "Primary Office Network", "Floor 7"),
    ("103.54.189.22", "Secondary ISP / Gateway", "Floor 9"),
    ("27.0.150.66", "Backup Office Lease Line", "Floor 17"),
]
...
def ensure_initial_office_ips(db: Session):
    """Ensure column schema and seed initial office IPs if empty."""
    try:
    ...
    if db.query(OfficeIP).first() is not None:
        return
...
def create_office_ip(
    ...
    """Add a new authorized office IP address (admin-only)."""
    # Check if IP already exists
    existing = db.query(OfficeIP).filter(OfficeIP.ip_address == payload.ip_address).first()
    if existing:
        raise HTTPException(status_code=400, detail=f"IP address '{payload.ip_address}' is already added.")
...
def update_office_ip(
    ...
    if payload.ip_address is not None and payload.ip_address != item.ip_address:
        # Check duplicate
        duplicate = db.query(OfficeIP).filter(OfficeIP.ip_address == payload.ip_address, OfficeIP.id != ip_id).first()
        if duplicate:
            raise HTTPException(status_code=400, detail=f"IP address '{payload.ip_address}' is already in use.")
        item.ip_address = payload.ip_address

    if payload.label is not None:
        item.label = payload.label.strip() if payload.label.strip() else None

    if payload.floor is not None:
        item.floor = payload.floor.strip() if payload.floor.strip() else None
