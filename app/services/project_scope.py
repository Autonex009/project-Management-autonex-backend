...
lead, and a manager's own request leaves the project entirely. This is decided by
designation rather than ``users.role`` so it holds wherever the person is allocated — a
program manager lent to another project as a temporary lead still escalates to an admin
rather than to that project's PM.
...
# frontend/src/utils/roleAccess.js. A tag is needed as well as the designation because a
# program manager on loan leads by virtue of the tag while keeping their own designation.
...
def project_lead_ids(db: Session, project: Optional[DailySheet]) -> set[int]:
    ...
    Leads are recorded as allocations, not as a column: leadership is per-project, which a
    designation cannot express — that is what allows a program manager to lead one project
    while managing another. Qualifying either by designation or by a tag on this specific
    allocation covers both the dedicated lead and the borrowed manager.
