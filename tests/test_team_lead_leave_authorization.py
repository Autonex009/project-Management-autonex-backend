...
def test_a_program_manager_acting_as_team_lead_escalates_to_admin(env):
    """Requirement: a PM lent to another project keeps admin as their own approver.

    PM B is allocated to project A as a temporary lead, so PM A manages a project PM B
    sits on — the designation rule is what stops PM A approving PM B's leave.
    """
    client, db, seeded = env
