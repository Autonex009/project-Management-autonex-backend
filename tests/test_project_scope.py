...
def test_the_role_tag_alone_makes_someone_a_lead(db):
    """The tag alone is enough, whatever the person's designation says.

    The Team Lead picker only offers Team Leads, but the Allocations page can tag any
    allocation — so the resolver must honour the tag rather than re-deriving from
    designation, or that data would resolve to no lead at all.
    """
...
def test_a_program_managers_own_request_is_admin_only(db):
    """Even when the two managers share a project.

    PM B is allocated to project A, which PM A manages — so without the designation rule
    PM A could approve a fellow manager's leave. Rank does not depend on how someone came
